...
gulp.task(pkg.name + '/build', function () {
    var normalWebpackStream = {
        ...
        normalStream = gulp.src('./src/index.js').pipe(webpackStream(normalWebpackStream)).pipe(gulp.dest('./dist'));
    ...
    var uglifyStream = gulp.src('./src/index.js').pipe(webpackStream(ulgifyWebpackStream)).pipe(gulp.dest('./dist'));

    return es.concat(normalStream, uglifyStream);
});

gulp.task(pkg.name + '/watch', function () {
    return gulp.watch(['./src/**/*.js', './src/**/*.json', './src/**/*.html'], [pkg.name + '/build']);
});