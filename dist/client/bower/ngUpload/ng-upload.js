...
//  });
//
angular.module('ngUpload', []).directive('uploadSubmit', ["$parse", function ($parse) {
  ...
  return {
    ...
    link: function link(scope, element, attrs) {
      ...
      element.bind('submit', function uploadStart($event) {
        ...
        if (options.convertHidden) {
          angular.forEach(element.find('input'), function (el) {
            ...
            if (_el.attr('ng-model') && _el.attr('type') && _el.attr('type') == 'hidden') {
      ...
      function uploadEnd() {
        ...
        try {
          var bodyContent = (iframe[0].contentDocument || iframe[0].contentWindow.document).body;
